...
const displayCompleted = computed(() =>
  mutation.isPending.value
    ? !props.todo.completed  // Optimistic: show toggled state
    : props.todo.completed   // Settled: show actual state
)
</script>
...
<template>
  <div>
    <input
      ...
      @change="mutation.mutate(todo.id)"
    />
    <span :style="{ opacity: mutation.isPending.value ? 0.5 : 1 }">
      {{ todo.title }}
    </span>
  </div>
</template>
