...
Uses Playwright's Chromium with a persistent profile at `~/.chromium-cdp` (retains logins/cookies across restarts).
...
All `cdp.mjs` commands MUST be prefixed with the port file env var:

```bash
CDP_PORT_FILE=~/.chromium-cdp/DevToolsActivePort scripts/cdp.mjs <command>
```
...
```bash
CDP_PORT_FILE=~/.chromium-cdp/DevToolsActivePort scripts/cdp.mjs list
```
